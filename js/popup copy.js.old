...
async function getTestURLS(tab){
  ...
  if (siteName !== 'other'){
    createPaths(url, mydiv, urlHost, lang);
  } 
...
function createPaths(url, mydiv, urlHost, lang){
  let linebreak = document.createElement("br");
  let paths = getPaths(url, lang);
...
function getPaths(url, lang) {
  let urls = [];
  //fetch the site url /wp-json/coolwhip/v2/priority-pages/
  var clwpAPIUrl = new URL('/wp-json/coolwhip/v2/priority-pages/', url).href;
